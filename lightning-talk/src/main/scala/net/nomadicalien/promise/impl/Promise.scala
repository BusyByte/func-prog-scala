...
private[promise] trait Promise[T] extends net.nomadicalien.promise.Promise[T] with Future[T] {
  def future: this.type = this

  import net.nomadicalien.promise.Future
  import net.nomadicalien.promise.impl.Promise.DefaultPromise
...
private[promise] object Promise {
  ...
  class DefaultPromise[T] extends AtomicReference[AnyRef](Nil) with Promise[T] {
    ...
    /** Link this promise to the root of another promise using `link()`. Should only be
      ...
      */
    protected[promise] final def linkRootOf(target: DefaultPromise[T]): Unit = link(target.compressedRoot())
  ...
  object KeptPromise {
    ...
    def apply[T](result: Try[T]): net.nomadicalien.promise.Promise[T] =
